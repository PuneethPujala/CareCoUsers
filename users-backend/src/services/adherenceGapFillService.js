...
function isMedicationActiveOnDate(med, dateStr, timezone) {
    ...
    const medStartStr = start ? moment(start).tz(timezone).format('YYYY-MM-DD') : null;
...
async function buildDailyAdherenceTimeline(patient, startDateStr, endDateStr) {
    ...
    const yesterdayStr = moment().tz(timezone).subtract(1, 'days').format('YYYY-MM-DD');
    ...
    while (!cursor.isAfter(limit)) {
        ...
        if (!dailyLogMap.has(dateStr)) {
            ...
            if (dateStr <= yesterdayStr && dateStr >= createdDateStr) {
                ...
                const activeMedsOnDate = allMedsRaw.filter(med => isMedicationActiveOnDate(med, dateStr, timezone));
    ...
    return Array.from(dailyLogMap.values()).sort((a, b) => a.date.localeCompare(b.date));
}
