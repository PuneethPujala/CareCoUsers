...
async function generateMorningBrief(patientId) {
    try {
        ...
        const history = await PatientHealthStateHistory.find({ patient_id: patientId })
            .sort({ date: -1 })
            .limit(2)
            .lean();
        ...
        const trajectoryLabel = trajectory === 'positive' || trajectory === 'improving' ? 'Improving' :
                                trajectory === 'negative' || trajectory === 'declining' ? 'Declining' : 'Stable';
        ...
        const recentVitalsText = insights?.risk_factors?.find(f => f.toLowerCase().includes('blood pressure') || f.toLowerCase().includes('reading'));
        if (recentVitalsText) {
        ...
        logger.error('[DailyBriefService] Error generating morning brief', { error: err.message, patientId });
        return null;
    }
}
...
async function generateCompanionBrief(patientId) {
    try {
        ...
        const statusLabel = riskLevel === 'high' ? 'Needs Attention' :
                            riskLevel === 'medium' ? 'Monitor Closely' : 'Stable';

        const recommendedActions = [];
        if (activeInterventions.length > 0) {
            activeInterventions.slice(0, 2).forEach(item => {
                if (item.type === 'medication_reminder') recommendedActions.push('Send medication reminder nudge');
                if (item.type === 'bp_request') recommendedActions.push('Request BP log sync');
                if (item.type === 'checkin_call') recommendedActions.push('Schedule check-in call');
                if (item.type === 'escalation_contact') recommendedActions.push('Urgent: recommend coordinator escalation');
            });
        }
        ...
        logger.error('[DailyBriefService] Error generating companion brief', { error: err.message, patientId });
        return null;
    }
}
