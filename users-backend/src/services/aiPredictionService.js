...
class AIPredictionService {
  ...
  static async processPatientPrediction(patientId) {
    try {
      ...
      const response = await axios.post(
        `${AI_VITALS_URL}/api/predict-vitals`,
        {
      ...
      let predictionDoc = await AIVitalPrediction.findOne({
        patient_id: patientId,
      });
  ...
  static calculateStreak(existingDoc, newLabel) {
    ...
    // Check if prediction was already run today to maintain idempotency on retries/multiple runs
