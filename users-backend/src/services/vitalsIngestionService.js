...
class VitalsIngestionService {
  ...
  static async processBatch(patientId, readings, source) {
    ...
    for (const reading of readings) {
      ...
      if (
        reading.blood_pressure?.systolic != null &&
        reading.blood_pressure?.diastolic != null
      ) {
  ...
  static _detectAnomaly(doc) {
    ...
    if (
      doc.oxygen_saturation != null &&
      doc.oxygen_saturation < DANGER_THRESHOLDS.oxygen_saturation.min
    ) {
