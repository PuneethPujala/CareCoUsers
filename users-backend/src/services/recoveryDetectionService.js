 ...
 */
function detectRecovery(history = [], currentRiskScore = 0, recentAlerts = [], careVisibility = 100) {
    ...
    const hasNetDecline = len > 1 && riskScores[len - 1] < riskScores[Math.max(0, len - 1 - recoveryDays)];
    ...
    const hasCriticalAlerts = recentAlerts.some(alert => {
        const isRecent = new Date(alert.created_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        const isCritical = alert.severity === 'critical' || alert.type === 'critical_vital';
        return isRecent && isCritical;
    });

    // Rule 3: Average adherence over last 7 days >= 75%
    const recentHistory = history.slice(-7);
    const adherenceSum = recentHistory.reduce((acc, h) => acc + (h.adherence?.today ?? 0), 0);
    const adherenceAvg = recentHistory.length > 0 ? (adherenceSum / recentHistory.length) : 100;
