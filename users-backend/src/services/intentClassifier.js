...
function classifyIntent(query) {
    ...
    if (q.includes("why did my score drop") || q.includes("why did my health score drop") || 
        q.includes("why did my score decrease") || q.includes("explain my score change") || 
        q.includes("why is my score lower") || q.includes("why did it drop")) {
        return 'score_drop';
    }

    if (q.includes("how can i improve my score") || q.includes("how to improve my score") || 
        q.includes("how to increase my score") || q.includes("how can i increase my health score") || 
        q.includes("improve my health score")) {
        return 'improve_score';
    }

    if (q.includes("what medications did i miss") || q.includes("what meds did i miss") || 
        q.includes("show missed doses") || q.includes("missed medications") || 
        q.includes("missed doses") || q.includes("did i miss any med")) {
        return 'missed_meds';
    }

    if (q.includes("how am i doing this week") || q.includes("weekly progress") || 
        q.includes("weekly summary") || q.includes("my progress this week") || 
...
async function resolveDeterministicIntent(patientId, intent) {
    try {
        ...
        const [patientContext, insights, historyHistory] = await Promise.all([
            ...
            PatientHealthStateHistory.find({ patient_id: patientId }).sort({ date: -1 }).limit(14).lean()
        ]);
        ...
        if (intent === 'score_drop') {
            ...
            if (historyHistory.length >= 2) {
                scoreChange = (historyHistory[0].score ?? 80) - (historyHistory[historyHistory.length - 1].score ?? 80);
            }

            const pointsChange = Math.abs(scoreChange);
            const dropText = scoreChange < 0 
            ...
            if (weeklyMissed > 0) {
                bullets.push(`• Missed ${weeklyMissed} scheduled medication ${weeklyMissed === 1 ? 'dose' : 'doses'} this week`);
            }

            // 2. Vitals sync check
            const lastBPDate = await VitalLog.findOne({ patient_id: patientId }).sort({ date: -1 }).lean();
            if (lastBPDate) {
            ...
            const todaySleepLog = await SleepLog.findOne({ patient_id: patientId, date: todayUtc }).lean();
            if (todaySleepLog) {
                const sleepDiff = sleepAvg - todaySleepLog.hours;
                if (sleepDiff > 1.0) {
                    bullets.push(`• Today's sleep decreased by ${sleepDiff.toFixed(1)} hours compared to your ${sleepAvg}h average`);
                }
            } else {
            ...
            if (bullets.length === 0) {
                bullets.push("• Vitals and medications are otherwise stable. Continue logs to improve your metrics!");
            }
        ...
        if (intent === 'improve_score') {
            ...
            if (weeklyAdherence < 75) {
                targets.push(`• Complete your scheduled medications today (target: 75%+ compliance, currently ${weeklyAdherence}%)`);
            } else {
                targets.push(`• Keep up your medication routine to maintain your perfect streak`);
            }

            // 2. BP request
            const threeDaysAgo = nowLocal.clone().subtract(3, 'days').startOf('day').toDate();
            const BPLogged = await VitalLog.exists({ patient_id: patientId, date: { $gte: threeDaysAgo } });
            if (!BPLogged) {
            ...
            // 3. Sleep targets
            const sleepAvg = await computeSleepTarget(patientId, timezone);
            targets.push(`• Sleep at least ${sleepAvg} hours tonight to meet your 14-day baseline`);
            ...
            return {
                ...
                cards: [{
                    type: 'medications',
                    taken: (patientContext?.today_status?.medicines || []).filter(m => m.taken).map(m => m.name),
                    remaining: (patientContext?.today_status?.medicines || []).filter(m => !m.taken).map(m => `${m.name} (${m.time_slot})`)
                }]
            };
        }
        ...
        if (intent === 'missed_meds') {
            ...
            const sevenDaysAgo = nowLocal.clone().subtract(7, 'days').startOf('day').toDate();
            ...
            missedLogs.forEach(log => {
                const missedList = (log.medicines || []).filter(m => !m.taken && m.is_active !== false);
            ...
            let replyText = '';
            if (keys.length > 0) {
                replyText = "In the past 7 days, you missed the following scheduled doses:\n\n" +
                    keys.map(k => `• ${k}: missed ${missedCounts[k]} ${missedCounts[k] === 1 ? 'time' : 'times'}`).join('\n');
            } else {
                replyText = "Great news! You have not missed any scheduled medication doses in the past 7 days. Keep it up! 🎉";
            }
        ...
        if (intent === 'weekly_summary') {
            ...
            const momentumDirection = momentum >= 0 ? `+${momentum} Improving` : `${momentum} Declining`;

            const replyText = `Here is your Health Intelligence Weekly Summary:\n\n` +
        ...
        logger.error('[IntentClassifier] Error resolving deterministic intent', { error: err.message, intent, patientId });
        return {
