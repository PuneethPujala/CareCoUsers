...
async function computeSleepTarget(patientId, timezone) {
    ...
    const sleepLogs7 = sleepLogs14.filter(log => new Date(log.date) >= sevenDaysAgo);
...
async function getOrGenerateCarePlan(patientId) {
    try {
        ...
        medicationTasks.sort((a, b) => a.name.localeCompare(b.name) || a.time_slot.localeCompare(b.time_slot));
        ...
        if (activePlan) {
            ...
            const medsChanged = JSON.stringify(activePlan.medication_tasks.map(t => ({ name: t.name, time_slot: t.time_slot })).sort((a, b) => a.name.localeCompare(b.name) || a.time_slot.localeCompare(b.time_slot))) !== JSON.stringify(medicationTasks);
            const sleepChanged = Math.abs(activePlan.sleep_hours_goal - sleepTarget) > 0.1;
            ...
            logger.info(`[CarePlanService] Upgraded Care Plan for patient ${patientId} to Version ${nextVersion} due to changes.`);
            return newPlan;
        }
        ...
        logger.info(`[CarePlanService] Generated new Initial Care Plan for patient ${patientId}`);
        return initialPlan;

    } catch (err) {
        logger.error('[CarePlanService] Error in getOrGenerateCarePlan', { error: err.message, patientId });
        return null;
    }
}
