...
async function issueTokenPair(
  { userId, userType, subject, role, email, emailVerified },
  req
) {
  const accessPayload = {
    sub: subject,
  ...
  return {
    ...
    expires_at:
      ...
      parseExpiresInSeconds(jwtConfig.accessExpiresIn),
  };
}
...
async function revokeAllSessionsGlobally(userId) {
  ...
  try {
    ...
    console.warn(
      `Failed to set global invalidation for ${userId}:`,
      err.message
    );
  }
}
...
async function checkRedisSessionValidity(token, payload) {
  ...
  if (
    globalTimestamp &&
    payload.iat &&
    payload.iat < parseInt(globalTimestamp, 10)
  ) {
    return false;
  }
