...
function scoreLifestyle(lifestyle, bracket) {
    ...
    return { pts: Math.min(20, pts), note: `${smoking || '?'}/${alcohol || '?'}/${ex || '?'}` };
}
...
function scoreConditions(conditions, allergies) {
    ...
    return { pts: Math.max(0, pts), note: `${activeConditions.length} active conditions` };
}
...
function scoreVitals(latestVitals, lifestyle, bracket) {
    ...
    if (hr) {
        const hrOk = bracket === 'elderly' ? (hr >= 50 && hr <= 100) : (hr >= 55 && hr <= 95);
        if (hrOk) pts += 3;
    ...
    if (sys && dia) {
        const bpNormal = bracket === 'elderly'
...
function scorePreventiveCare(patient) {
    ...
    const upcomingAppt = (patient.appointments || []).find(a => a.status === 'upcoming');
    if (upcomingAppt) pts += 2;
    const hasEmergencyContact = (patient.trusted_contacts || []).some(c => c.is_emergency);
...
function gradeFromScore(score) {
    ...
    if (score >= 70) return { grade: 'B', label: 'Managing Well', color: '#3B82F6' };
...
function generateTips(patient, breakdown, bracket, adherenceRate, latestVitals) {
    const lifestyle = patient.lifestyle || {};
    const tips = [];

    /* helpers */
    const pct = (dim) => breakdown[dim] ? Math.round((breakdown[dim].pts / breakdown[dim].max) * 100) : 100;
    ...
    if (weak('adherence', 85) && adherenceRate !== null) {
        if (adherenceRate < 50) {
            push({
                category: 'adherence', priority: 1, impact: 'high',
                icon: '⏰',
            ...
            push({
                category: 'adherence', priority: 2, impact: 'medium',
                icon: '✅',
    ...
    if (lifestyle.smoking_status === 'current') {
        push({
            category: 'lifestyle', priority: 1, impact: 'high',
            icon: '🚭',
    ...
    if (lifestyle.alcohol_use === 'heavy') {
        ...
        push({
            category: 'lifestyle', priority: 3, impact: 'low',
            icon: '💧',
    ...
    if (lifestyle.exercise_frequency === 'none' || lifestyle.exercise_frequency === 'light') {
        if (bracket === 'elderly') {
            push({
                category: 'lifestyle', priority: 2, impact: 'medium',
                icon: '🧘',
            ...
            push({
                category: 'lifestyle', priority: 2, impact: 'high',
                icon: '🏃',
            ...
            push({
                category: 'lifestyle', priority: 3, impact: 'medium',
                icon: '🚶',
    ...
    if (h && w && h > 0) {
        ...
        if (bmi >= 28 && bracket !== 'elderly') {
            push({
                category: 'vitals', priority: 2, impact: 'medium',
                icon: '🥗',
    ...
    if (!latestVitals && weak('vitals')) {
        push({
            category: 'vitals', priority: 3, impact: 'medium',
            icon: '🩺',
    ...
    if (weak('preventive', 60)) {
        if (!patient.gp_name) {
            push({
                category: 'preventive', priority: 2, impact: 'medium',
                icon: '👨‍⚕️',
        ...
        if (!(patient.vaccinations || []).length) {
            push({
                category: 'preventive', priority: 3, impact: 'low',
                icon: '💉',
    ...
    if (activeConditions.length >= 2) {
        push({
            category: 'conditions', priority: 2, impact: 'medium',
            icon: '📓',
    ...
    if (allGood) {
        push({
            category: 'general', priority: 3, impact: 'low',
            icon: '⭐',
...
function computeHealthScore(patient, adherenceRate, latestVitals) {
    ...
    const rawScore =
        adherence.pts + (adherence.bonus || 0) +
        lifestyle_.pts +
        conditions.pts +
        vitals.pts +
        preventive.pts +
    ...
    const tips = generateTips(patient, breakdown, bracket, adherenceRate, latestVitals);
