      ...
      if (!isNaN(deviceTime)) {
        ...
        if (deviceTz) {
          if (isValidTimeZone(deviceTz)) {
            ...
            console.warn(`[CHAOS_CLOCK_DRIFT_V1] Malformed/Invalid timezone header: "${deviceTz}". Falling back to UTC.`);
          }
        }

        // If clock drift is >= 5 seconds (5000ms)
        if (driftMs >= 5000) {
          logger.warn('[CHAOS_CLOCK_DRIFT_V1] Significant device clock drift detected', {
            drift_ms: driftMs,
        ...
        console.warn(`[CHAOS_CLOCK_DRIFT_V1] Malformed date in x-device-timestamp header: "${deviceTimeStr}"`);
      }
    }
  } catch (err) {
