  ...
  try {
    ...
    for (const pt of patients) {
      if (
        ...
        AIPredictionService.processPatientPrediction(pt._id.toString()).catch(
          (e) => {
            console.error(
              `In-process prediction failed for patient ${pt._id}:`,
              e,
            );
          },
        );
      } else {
        await vitalsPredictionQueue.add("predict", {
          patientId: pt._id.toString(),
        });
      }
    }
    console.log(
      `Successfully dispatched/queued ${patients.length} prediction jobs.`,
    );
  } catch (e) {
