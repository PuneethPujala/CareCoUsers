  ...
  for (const patient of patients) {
    try {
      ...
      const endHour = parseInt(
        (limits.quiet_hours_end || '08:00').split(':')[0]
      );
      const startHour = parseInt(
        (limits.quiet_hours_start || '21:00').split(':')[0]
      );
      ...
      console.error(
        `[AI-Notification] Error evaluating patient ${patient._id}:`,
        err.message
      );
    }
  }
...
function startNotificationCron() {
  ...
  console.log(
    '🤖 AI Health Companion Cron started (Hourly evaluating patient timezones).'
  );
}
