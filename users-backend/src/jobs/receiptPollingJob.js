...
const pollPushReceipts = async () => {
    ...
    console.log(`[Job] Push Receipt Polling started at ${new Date().toISOString()}`);
    ...
    try {
        ...
        console.log(`[Job] Polling receipts for ${ticketIds.length} unique ticket(s)...`);
        ...
        for (let i = 0; i < ticketIds.length; i += batchSize) {
            ...
            try {
                const response = await axios.post(EXPO_RECEIPT_URL, {
                    ids: batchIds
                }, {
                    headers: {
                ...
                if (!receipts || typeof receipts !== 'object') {
                    console.warn('[Job] Malformed or empty receipts response from Expo:', response.data);
                    continue;
                }
                ...
                for (const ticketId of batchIds) {
                    ...
                    if (receipt.status === 'ok') {
                        ...
                        const errorMsg = receipt.details?.error || receipt.message || 'Unknown error';
                        console.error(`[Job] Receipt error for ticket ${ticketId}: ${errorMsg}`);
                        ...
                        const terminalErrors = ['DeviceNotRegistered', 'InvalidCredentials', 'MessageTooBig'];
                        if (terminalErrors.includes(receipt.details?.error)) {
                            ...
                            if (patientId && loggedToken) {
                                const patient = await Patient.findById(patientId).select('expo_push_token expo_push_token_failures');
                                ...
                                if (patient && patient.expo_push_token === loggedToken) {
                                    const nextFailures = (patient.expo_push_token_failures || 0) + 1;
                                    if (nextFailures >= 3) {
                                        console.warn(`[Job] Pruning token for patient ${patientId} after 3 consecutive failures.`);
                                        ...
                                        console.log(`[Job] Incremented token failures for patient ${patientId} to ${nextFailures}.`);
                                    }
                                }
                            }
                        }
                ...
                console.error(`[Job] Error polling receipt batch starting at index ${i}:`, batchErr.message);
            }
        }
    } catch (error) {
        console.error('[Job] Push Receipt Polling failed:', error);
    } finally {
        console.log(`[Job] Push Receipt Polling finished in ${Date.now() - startTime}ms`);
    }
};
...
const pruneDeadDevices = async () => {
    ...
    try {
        ...
        console.log(`[Job] Dead Device Sweep completed. Pruned ${result.modifiedCount} inactive device token(s).`);
    } catch (error) {
        console.error('[Job] Dead Device Sweep failed:', error);
    } finally {
        console.log(`[Job] Dead Device Sweep finished in ${Date.now() - startTime}ms`);
    }
};
...
const startReceiptCron = () => {
    if (isReceiptCronStarted) {
        console.warn('⚠️ Push receipt cron already started. Skipping duplicate initialization.');
        return;
    }
