...
const vitalsValidators = [
  body("patient_id")
    ...
    .isMongoId()
    .withMessage("patient_id must be a valid ObjectId"),
  body("heart_rate")
    .notEmpty()
    .withMessage("heart_rate is required")
    .isFloat({ min: 30, max: 250 })
    .withMessage("heart_rate must be 30–250 bpm"),
  body("blood_pressure.systolic")
    .notEmpty()
    .withMessage("blood_pressure.systolic is required")
    .isFloat({ min: 60, max: 250 })
    .withMessage("systolic must be 60–250 mmHg"),
  body("blood_pressure.diastolic")
    .notEmpty()
    .withMessage("blood_pressure.diastolic is required")
    .isFloat({ min: 40, max: 150 })
    .withMessage("diastolic must be 40–150 mmHg"),
  body("oxygen_saturation")
    .notEmpty()
    .withMessage("oxygen_saturation is required")
    .isFloat({ min: 0, max: 100 })
    .withMessage("oxygen_saturation must be 0–100%"),
  body("hydration")
    .notEmpty()
    .withMessage("hydration is required")
    .isFloat({ min: 0, max: 100 })
    .withMessage("hydration must be 0–100%"),
  body("date")
    .optional()
    .isISO8601()
...
router.post("/", vitalsValidators, validate, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Vitals saved successfully", vitals: vitalLog });
  } catch (error) {
