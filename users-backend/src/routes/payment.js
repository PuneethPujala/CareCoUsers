...
router.post("/webhook", async (req, res) => {
  try {
    ...
    if (!provider) {
      return res
        .status(400)
        .json({ error: "Webhook provider could not be determined." });
    }
    ...
    if (provider === "stripe") {
      ...
      try {
        ...
        try {
          ...
          await triggerSystemAlert(
            ...
            `Stripe signature check failed: ${err.message}. IP: ${req.ip}`,
          );
        } catch (e) {
          logger.error(
            "Failed to log Stripe signature verification failure:",
            e,
          );
        }
        return res
      ...
      if (!signature) {
        return res
          .status(400)
          .json({ error: "Missing x-razorpay-signature header" });
      }
      const secret =
      ...
      if (signature !== expectedSignature) {
        ...
        try {
          ...
          await triggerSystemAlert(
            ...
            `Razorpay signature check failed. IP: ${req.ip}`,
          );
        } catch (e) {
          logger.error(
            "Failed to log Razorpay signature verification failure:",
            e,
          );
        }
    ...
    if (provider === "stripe") {
      ...
      if (eventType === "checkout.session.completed") {
        ...
        patientId =
          sessionObj?.client_reference_id || sessionObj?.metadata?.patientId;
        planId = sessionObj?.metadata?.planId || "basic";
      }
    } else if (provider === "razorpay") {
      eventId =
        eventObj.id ||
        eventObj.payload?.payment?.entity?.id ||
    ...
    if (activationEvents.includes(eventType)) {
      if (!patientId) {
        logger.warn("Patient ID missing in webhook metadata", {
          eventId,
          eventType,
        });
        return res
          .status(400)
          .json({ error: "Patient ID missing in webhook metadata" });
      }

      const patient = await Patient.findById(patientId);
      if (!patient) {
        logger.error("Patient not found for webhook activation", {
          patientId,
          eventId,
        });
      ...
      logger.info("Webhook subscription activation successful", {
        patientId: updatedPatient._id,
        eventId,
      });
    } else {
      logger.info("Non-activation webhook event processed", {
        eventId,
        eventType,
      });
    }
