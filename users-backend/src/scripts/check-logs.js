...
async function run() {
  ...
  recentPatients.forEach((p) => {
    console.log(
      `Email: ${p.email}, Uid: ${p.supabase_uid}, CreatedAt: ${p.createdAt}, Active: ${p.is_active}`,
    );
  });
  ...
  recentProfiles.forEach((p) => {
    console.log(
      `Email: ${p.email}, Uid: ${p.supabaseUid}, Role: ${p.role}, CreatedAt: ${p.createdAt}`,
    );
  });
  ...
  try {
    ...
    logs.forEach((l) => {
      console.log(
        `Timestamp: ${l.timestamp}, Event: ${l.eventType}, Subject: ${l.subjectId || l.subject}, UserType: ${l.userType}, Details: ${JSON.stringify(l.details)}`,
      );
    });
  } catch (e) {
