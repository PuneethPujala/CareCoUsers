...
async function run() {
  ...
  try {
    ...
    const loginRes = await authService.login(
      { email: testEmail, password: testPassword, role: "patient" },
      { ip: "127.0.0.1", headers: {} },
    );

    if (loginRes.profile && loginRes.profile.role === "companion") {
      console.log(
        '✅ Success! Login correctly resolved to "companion" role automatically.\n',
      );
    } else {
      throw new Error(
        `Failed: Resolved to incorrect profile role: ${loginRes.profile?.role}`,
      );
    }
    ...
    if (oauthRes.profile && oauthRes.profile.role === "companion") {
      console.log(
        "✅ Success! OAuth register successfully linked to existing Companion instead of creating a Patient.\n",
      );
    } else {
      throw new Error(
        `Failed: OAuth registration resolved to incorrect role or failed linking.`,
      );
    }
    ...
    if (duplicatePatient) {
      throw new Error(
        "Failed: Duplicate Patient record was mistakenly created for a Companion email.",
      );
    }
    console.log(
      "✅ Success! Double-check confirmed no duplicate Patient record exists.\n",
    );
    ...
    try {
      ...
      throw new Error(
        "Failed: Allowed overwriting an already linked Supabase Uid!",
      );
    } catch (err) {
      if (err.code === "OAUTH_LINK_CONFLICT") {
        console.log(
          "✅ Success! Prevented link hijacking with OAUTH_LINK_CONFLICT guard.\n",
        );
      } else {
        throw err;
      }
    ...
    try {
      await authService.login(
        { email: oauthOnlyEmail, password: "AnyPassword1!", role: "patient" },
        { ip: "127.0.0.1", headers: {} },
      );
      throw new Error(
        "Failed: Allowed password login on password-less account.",
      );
    } catch (err) {
      if (err.code === "NO_PASSWORD_SET") {
        console.log(
          "✅ Success! Returned clean NO_PASSWORD_SET code without crashing.\n",
        );
      } else {
        throw err;
      }
