...
describe('Patient Health State V2 & Event-Driven Queue', () => {
    ...
    describe('enqueueHealthStateRecompute', () => {
        ...
        it('should fall back to synchronous recompute if healthStateQueue.add throws an error', async () => {
            healthStateQueue.add.mockRejectedValue(new Error('Redis Connection Failure'));
    ...
    describe('BullMQ Integration (Requires Redis)', () => {
        ...
        beforeAll(async () => {
            ...
            try {
                ...
                console.log('⚠️ Local Redis is offline. Skipping BullMQ integration tests.');
            } finally {
                await client.quit();
            }
        });
        ...
        it('should deduplicate multiple delayed jobs with the same jobId and allow re-enqueueing after completion', async () => {
            ...
            const job1 = await testQueue.add('recompute', { patientId: 'p1' }, { jobId, delay: 500, removeOnComplete: true, removeOnFail: true });
            const job2 = await testQueue.add('recompute', { patientId: 'p1' }, { jobId, delay: 500, removeOnComplete: true, removeOnFail: true });
            ...
            const job3 = await testQueue.add('recompute', { patientId: 'p1' }, { jobId, delay: 10, removeOnComplete: true, removeOnFail: true });
