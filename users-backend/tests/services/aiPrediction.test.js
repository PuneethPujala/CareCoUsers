...
function makeMockPredictionDoc(overrides = {}) {
  return {
    ...
    predictions: [],
    updated_at: new Date(Date.now() - 24 * 60 * 60 * 1000), // last updated yesterday
...
describe("AIPredictionService", () => {
  ...
  describe("calculateStreak", () => {
    ...
    it("should NOT increment streak if prediction was already run today (idempotency)", () => {
      const doc = makeMockPredictionDoc({
        ...
        updated_at: new Date(), // updated today
      });
  ...
  describe("shouldSendAlert", () => {
    ...
    it("should follow exact alert matrix for 15 consecutive critical days", () => {
      ...
      for (const [day, shouldAlert] of Object.entries(expected)) {
        expect(
          AIPredictionService.shouldSendAlert(Number(day), "Critical"),
        ).toBe(shouldAlert);
      }
    });
  });
  ...
  describe("processPatientPrediction", () => {
        ...
        data: {
          ...
          predictions: [
            ...
            },
            {
              date: "2024-01-10",
              heart_rate: 73,
              blood_pressure: { systolic: 121, diastolic: 81 },
    ...
    it("should handle AI service timeout/error gracefully", async () => {
      ...
      axios.post.mockRejectedValue(
        new Error("ECONNREFUSED: AI service is offline"),
      );
    ...
    it("should notify caller when patient has assigned_caller_id", async () => {
      ...
      PushNotificationService.sendCallerCriticalAlert.mockResolvedValue({
        success: true,
      });
      ...
      expect(result.notified).toBe(true);
      expect(
        PushNotificationService.sendCallerCriticalAlert,
      ).toHaveBeenCalledWith(mockCaller, mockPatient, expect.any(Object));
    });
  });
  ...
  describe("PushNotificationService (imported utility)", () => {
    ...
    const RealPushService = jest.requireActual(
      "../../src/utils/pushNotifications",
    );

    it("should validate correct Expo push token format", () => {
      expect(
        RealPushService.isValidExpoPushToken("ExponentPushToken[abc123]"),
      ).toBe(true);
      expect(RealPushService.isValidExpoPushToken("ExpoPushToken[xyz]")).toBe(
        true,
      );
    });
    ...
    it("should return failure for invalid token format", async () => {
      const result = await RealPushService.sendPush(
        "bad-token",
        "Title",
        "Body",
      );
