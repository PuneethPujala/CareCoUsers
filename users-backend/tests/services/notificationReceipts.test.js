...
const { pollPushReceipts, pruneDeadDevices } = require('../../src/jobs/receiptPollingJob');
...
describe('Notification Delivery Receipts & Token Pruning', () => {
    ...
    describe('pollPushReceipts', () => {
        ...
        it('should process partial batch failures correctly without crashing', async () => {
            ...
            const mockNotif1 = { patient_id: patientId1, expo_ticket_id: 'ticket-1', expo_push_token: 'token-1' };
            const mockNotif2 = { patient_id: patientId2, expo_ticket_id: 'ticket-2', expo_push_token: 'token-2' };
            const mockNotif3 = { patient_id: patientId3, expo_ticket_id: 'ticket-3', expo_push_token: 'token-3' };

            Notification.find.mockReturnValue({
                select: jest.fn().mockResolvedValue([mockNotif1, mockNotif2, mockNotif3])
            });
            ...
            expect(Notification.updateMany).toHaveBeenCalledWith(
                ...
                expect.objectContaining({
                    $set: expect.objectContaining({ expo_receipt_status: 'ok', push_delivered: true })
                })
            );
            expect(Patient.findByIdAndUpdate).toHaveBeenCalledWith(patientId1, { $set: { expo_push_token_failures: 0 } });

            // Verifies failing ticket-2 updates
            expect(Notification.updateMany).toHaveBeenCalledWith(
                { expo_ticket_id: 'ticket-2' },
                expect.objectContaining({
                    $set: expect.objectContaining({ expo_receipt_status: 'error', expo_receipt_error: 'DeviceNotRegistered' })
                })
            );
            expect(Patient.findByIdAndUpdate).toHaveBeenCalledWith(patientId2, { $set: { expo_push_token_failures: 1 } });
