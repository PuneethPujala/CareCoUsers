...
export default function PatientDashboard({ navigation }) {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            const { data } = await apiService.patients.getById(user.id || user._id);
            const patData = data.patient || data;
            const fetchedMeds = data.medications || patData.metadata?.medications || [];
            
            setAllMedications(fetchedMeds);
            const mappedConditions = (patData.metadata?.conditions || []).map((c, i) => ({
                id: i, condition: c, diagnosedDate: patData.createdAt, severity: 'Moderate', status: 'active'
            }));
            ...
            const phaseMeds = fetchedMeds.filter(med => {
                const times = med.scheduledTimes || [];
                if (phase === 'Morning') return times.some(t => t.includes('AM'));
                if (phase === 'Afternoon') return times.some(t => {
                    if (!t.includes('PM')) return false;
                    let h = parseInt(t.split(':')[0]);
                    if (h === 12) h = 0;
                    return h < 5;
                });
                if (phase === 'Night') return times.some(t => {
                    if (!t.includes('PM')) return false;
                    let h = parseInt(t.split(':')[0]);
            ...
            const mappedPhaseMeds = phaseMeds.map(m => {
                const isTaken = (m.takenLogs || []).some(log => log.date.split('T')[0] === todayStr);
                return {
                    ...m,
                    id: m._id,
                    name: m.name,
                    time: m.scheduledTimes?.length ? m.scheduledTimes.join(', ') : 'Daily',
                    taken: isTaken
                };
            });
            setMeds(mappedPhaseMeds);
    ...
    const toggleMed = async (id) => {
        ...
        try {
            ...
            await apiService.patients.toggleMedication(user.id || user._id, id, todayStr, timeStr);
        } catch (error) {
            ...
            <ScrollView style={s.body} contentContainerStyle={{ paddingBottom: 32 }} showsVerticalScrollIndicator={false}
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        {/* Time Phase Medications Progress */}
                        <View>
                            <View style={s.sectionHeader}>
                                <Text style={s.sectionTitle}>{currentPhase} Medications</Text>
                                <View style={s.headerActions}>
                                    <StatusBadge label={`${takenCount}/${meds.length} taken`} variant={takenCount > 0 && takenCount === meds.length ? 'success' : 'info'} />
                                </View>
                            </View>
                            <View style={s.progressBar}>
                                <View style={[s.progressFill, { width: meds.length > 0 ? `${(takenCount / meds.length) * 100}%` : '0%' }]} />
                            </View>
                            <PremiumCard style={{ padding: 0, marginTop: Spacing.md }}>
                                {meds.length === 0 && (
                                    <View style={{ padding: Spacing.lg, alignItems: 'center' }}>
                                        <Text style={{ color: Colors.textMuted }}>No medications scheduled for {currentPhase}.</Text>
                                    </View>
                                )}
                                {meds.map((m, i) => (
